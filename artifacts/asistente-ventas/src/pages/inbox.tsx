...
const RE_PAYMENT = /bizum|transfer|paypal|pago|paga|ingres|cuenta|iban/;
const RE_DOUBT =
    ...
    <div className="flex flex-col h-full bg-background">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="text-xs text-muted-foreground tabular-nums shrink-0">{formatPrice(car.price)}</div>
            </div>
        ...
        {(car.status === "open" || car.status === "released") && lead.depositPaid === false && (
          <div className="mt-2 flex">
            <Button size="sm" variant="secondary" onClick={onLock} disabled={lockCar.isPending}>
              <Lock className="h-3.5 w-3.5 mr-1.5" /> Bloquear 2h para este lead
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-border bg-card px-3 sm:px-4 py-3 space-y-2.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          >
            <Wand2 className="h-3 w-3" /> Borrador IA · Auto
          </Button>
          <Button
            variant="ghost"
            size="sm"
            className="h-7 text-[11px] gap-1 text-muted-foreground hover:text-foreground"
            onClick={() => setShowMoreAi((s) => !s)}
          >
            {showMoreAi ? "Ocultar opciones" : "Más sugerencias"}
            <ChevronDown className={cn("h-3 w-3 transition-transform", showMoreAi && "rotate-180")} />
          </Button>
          {showMoreAi && (
            <>
              {INTENTS.map((it) => (
